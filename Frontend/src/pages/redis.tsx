...
import { Database, Zap, Activity, Users, Clock, Trash2, RefreshCw, CheckCircle2, XCircle, BarChart3, Terminal } from "lucide-react";
...
export default function RedisPage() {
    ...
    const { data, isLoading, error, refetch } = useQuery<RedisStatus>({
        ...
        return (
            <div className="flex flex-col min-h-screen bg-background">
                <div className="flex-1 p-6 max-w-6xl mx-auto w-full">
                    <div className="h-8 w-48 bg-muted/40 rounded-lg animate-pulse mb-8" />
            ...
            <div className="border-b border-border bg-background/80 backdrop-blur-sm sticky top-0 z-10">
                <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-base font-semibold text-foreground">Redis Cache</h1>
                            <p className="text-xs text-muted-foreground">Cache, sessions, and real-time data store</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        {data?.version && (
                            <Badge variant="secondary" className="text-xs font-mono">v{data.version}</Badge>
                        )}
                ...
                {!connected ? (
                    <Card className="border-destructive/30 bg-destructive/5">
                        <CardContent className="p-6 text-center">
                            ...
                            <p className="text-sm font-medium text-foreground">Redis not connected</p>
                    ...
                    <>
                            ...
                            <CardContent className="p-5">
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                                    <div className="flex flex-col gap-1 p-3 rounded-lg bg-muted/30 border border-border/50">
                                        ...
                                        <span className="text-foreground font-mono text-xs">overview · stats · tables · activity</span>
                                        <span className="text-xs text-muted-foreground">5 – 30 second TTLs</span>
                                    </div>
                                    <div className="flex flex-col gap-1 p-3 rounded-lg bg-muted/30 border border-border/50">
                                        <span className="text-xs text-muted-foreground font-medium">Docker</span>
                                        <span className="text-foreground font-mono text-xs">containers · status</span>
                                        <span className="text-xs text-muted-foreground">4 – 8 second TTLs</span>
                                    </div>
                                    <div className="flex flex-col gap-1 p-3 rounded-lg bg-muted/30 border border-border/50">
                                        <span className="text-xs text-muted-foreground font-medium">System Stats</span>
                                        <span className="text-foreground font-mono text-xs">cpu · memory · storage</span>
                                        <span className="text-xs text-muted-foreground">3 second TTL + 60-pt history</span>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="border border-border">
                            ...
                            <CardContent className="p-0">
                                {sessions.length === 0 ? (
                                    <div className="p-5 text-center text-xs text-muted-foreground">No active sessions</div>
                                ) : (
                        ...
                        <Card className="border border-border">
                            ...
                            <CardContent className="p-0">
                                {queries.length === 0 ? (
                                    <div className="p-5 text-center text-xs text-muted-foreground">No queries logged yet</div>
                                ) : (
                                    <>
                                        <div className="divide-y divide-border">
