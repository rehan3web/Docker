...
import { cacheGet, cacheSet, pushStatsHistory, getStatsHistory } from '../lib/redis';
...
async function sampleStats() {
    try {
        ...
        const point = {
            ...
            load: Math.round((cpu.avgLoad ?? cpu.currentLoad ?? 0) * 100) / 100,
        };
        history.push(point);
        if (history.length > HISTORY_LIMIT) history.shift();
        pushStatsHistory(point);
...
router.get('/stats', authenticateToken, async (_req, res) => {
    const cached = await cacheGet<any>('system:stats');
    ...
    try {
        ...
        // Merge in-memory history with Redis-persisted history (Redis wins if longer)
        const redisHistory = await getStatsHistory();
        const mergedHistory = redisHistory.length >= history.length ? redisHistory : history;
        ...
        await cacheSet('system:stats', data, 3);
        res.json(data);
    } catch (err: any) {
